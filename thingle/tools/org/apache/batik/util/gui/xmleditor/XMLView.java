...
/**
 * View that uses the lexical information to determine the style characteristics of the text that it renders. This simply
 * colorizes the various tokens and assumes a constant font family and size.
 * 
	...
	protected int drawUnselectedText (Graphics g, int x, int y, int p0, int p1) throws BadLocationException {
		...
		while (pos < p1) {
			if (lastCtx != ctx) {
				...
				Color bgColor = context.getSyntaxBackground(lastCtx);
				if (bgColor != null) {
					g.setColor(bgColor);
					FontMetrics fontMetrics = g.getFontMetrics();
					int width = Utilities.getTabbedTextWidth(text, fontMetrics, x, this, mark);
					g.fillRect(x, y - fontMetrics.getHeight() + fontMetrics.getDescent(), width, fontMetrics.getHeight());
				}
